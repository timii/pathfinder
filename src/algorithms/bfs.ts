...
import { getAllNeighbourFieldPositions, getFieldPosition } from "./utils";
...
export function bfs(grid: IField[][]) {
    ...
    if (startNode) {
        const neighbours = getAllNeighbourFieldPositions(grid, startNode.firstIndex, startNode.secondIndex)
